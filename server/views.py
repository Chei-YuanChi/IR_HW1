...
def index(request):
    ...
    data : for outputing(include article, context, number of characters, words, and phases)
    ...
    num_words,num_phases,num_ch,num_search: caculate number of characters, words, and phases
    '''
    msg = data = file  = user_search  = search_err = new_title = delete_msg = ""
    num_words = num_phases = num_ch = num_search = 0
    words = phases = ch = []
                    ...
                    for context in titles.findall(".//AbstractText"): #get the abstracts
                        for i in context.text:
                            if ord(i) < 127 and ord(i) >= 33:
                                the_char += 1
                        phases = nltk.sent_tokenize(context.text) 
                        sentences = []
                        ...
                        if user_search != "":
                            ...
                            if label != "None":
                                ...
                                text += [['[' + label + ']', phases]] #get the labels and phases in the abstract
                            else:
                                text += [['', phases]] 
                        else:
                            if label != "None": 
                        ...
                        the_phases += len(phases)
                    num_ch += the_char
                    num_search += the_search
                    if user_search == "":
                        data += [[new_title, text, the_char, the_words, the_phases]]
                    else:
                        data += [[new_title, text, the_char, the_words, the_phases, the_search]]
            ...
            with open(os.path.join('files/', file), encoding="utf-8") as json_file:
                ...
                if user_search == "" or user_search == " ":
                    for user in datas:
                        ...
                        for i in text:
                            if ord(i) < 127 and ord(i) >= 33:
                                the_char += 1
                        num_ch += the_char
                        phases = nltk.sent_tokenize(text)
                        num_phases += len(phases)
                        the_phases += len(phases)
                        words = text.split( )
                        num_words += len(words)
                        ...
                        num_search += the_search
                        if user_search == "":
                            data += [[new_user, text, the_char, the_words, the_phases]]
                        else:
                            data += [[new_user, text, the_char, the_words, the_phases, the_search]]
                    ...
                    for user in datas:
                        ...
                        text = text.split('\n')
                        num_search += the_search
                        data += [[new_user, sentences, the_char, the_words, the_phases, the_search]]
            jsonfile = file
    return render(request,'index.html', locals())